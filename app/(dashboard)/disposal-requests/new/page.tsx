...
import { DisposalItemPicker } from "@/components/disposalRequests/DisposalItemPicker";
...
export default async function NewDisposalRequestPage() {
  ...
  // Already-disposed items can't be disposed again.
  const disposableItems = items.filter((item) => item.status !== "disposed");

  return (
    <div className="space-y-4">
      <Breadcrumb items={[{ label: selected.name }, { label: "Item Disposal" }, { label: "Create" }]} />
      <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">
        Create Disposal Request
      </h1>
      <p className="text-sm text-slate-500 dark:text-slate-400">
        Select one or more items below, then create a disposal request for them.
      </p>
      <DisposalItemPicker
