...
import type { DisposalRequest, DisposalRequestItem, Item, Property, User } from "@/lib/types";
...
export function DisposalRequestsTable({
  requests,
  requestItemsById,
  itemsById,
  usersById,
  ...
  const hotelColumn: ColumnDef<DisposalRequest> = {
    id: "hotel",
    header: "Hotel",
    cell: ({ row }) => propertiesById?.get(row.original.property_id)?.name ?? "Unknown",
  };

  const columns: ColumnDef<DisposalRequest>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          href={`/disposal-requests/${row.original.id}`}
          ...
          aria-label="View disposal request"
          title="View / decide"
        >
  ...
  return (
    <DataTable
      ...
      emptyMessage="No disposal requests yet."
    />
  );
}
